...
Ref is the set of abstract references to the heap. We do not care how exactly they look like. We assume the presence of a garbage collector (whether this is one we implement, or one that the host environment will provide, we do not distinguish on, at this point).
...
Here is an attempt to describe what a heap value from the POV of Flambda 2.0 looks like, in the most abstract sense:

A Value _v_ is a list of _heap_field_s.

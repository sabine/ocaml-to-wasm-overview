...
TODO: this is clearly wrong. _bound_variables_ should probably represent values on the stack, which can either be trivial values like _targetint_ and references to heap values, but never heap values directly!
...
_env'.variables[kinded_parameters_~i~_.variable]_ = _env.variables[args_~i~_]_
...
and let _imm_ = _env.variables[scrutinee]_ is an immediate of the target architecture.

Q: does that mean _simple_ is a type of variable that can hold only immediate values of the target architecture?

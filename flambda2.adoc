...
* `Singleton` _symbol_
* `Sets_of_closures (code_ids` Set(_code_id_), `closure_symbols` Map(_closure_id_ => _symbol_))*
...
On x86 this specializes to: the memory address where the machine instructions that correspond to the function lives.

On WASM, this specializes to: a `funcref` or an index into a global function table that refers to the WASM function that corresponds to the original function.
...
It seems that in Flambda 2.0, the closure representation does not contain the dreaded infix blocks anymore (which would have made the heap model more complex than this).
